...
Organizer.prototype.list = function (data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        listItem.setAttribute('data-main', data[i].index);
        var division = document.createElement("DIV");

        division.style.borderLeftWidth = "8px";
        division.style.borderLeftStyle = "solid";
        division.style.borderLeftColor = data[i].color;
        ...
        span.id = this.id + "-list-item-" + i + "-type";
        span.class = this.id + " type";
        span.appendChild(document.createTextNode(data[i].type));
...
Organizer.prototype.showEvents = function (data) {
    ...
    var eventNodes = document.getElementById("organizerContainer-list").children;
    if(eventNodes.length > 0) {
        for (let index = 0; index < eventNodes.length; index++) {
            const element = eventNodes[index];  
            element.onclick = () => {
                openDateModal(element.getAttribute("data-main"), true);
            }
        }
    }
};
        ...
        try {
            ...
            for (var key in month) {
                if (month[key].length > 0) {
                    ...
                    if(founded == true) {
                        allDays[key - 1].children[1].style.backgroundColor = 'red';
                    } else {
                        allDays[key - 1].children[1].style.backgroundColor = '#4e73df';
                    }
